...
try:
    ...
    st.set_page_config(page_title="TopSUNify", page_icon=img, layout="wide")
except:
        ...
        if st.session_state.get('last_processed_file') != file_id:
            try:
                with st.spinner("در حال ارسال فایل به موتور تحلیل مهندسی (FastAPI)..."):
                    # ایجاد یک فایل موقت پایداری که با Rerun حذف نشود (حذف خودکار با delete=False غیرفعال شده است)
                    import tempfile
...
with col_date1:
    # دریافت تاریخ امروز به صورت کاملاً خودکار از سیستم
    invoice_date = jdatetime.date.today()
    invoice_date_str = invoice_date.strftime("%Y/%m/%d")
    
    # نمایش تاریخ فاکتور به شکل باکس اطلاعاتی متنی غیرقابل کلیک و تغییر
    st.info(f"🗓️ تاریخ: {invoice_date_str}")

with col_date2:
    # محاسبه تاریخ سررسید دقیقاً ۳ روز جلوتر بر مبنای ماه شمسی
    due_date = invoice_date + jdatetime.timedelta(days=DUE_DAYS_DELTA)
    due_date_str = due_date.strftime("%Y/%m/%d")
    
    # نمایش تاریخ سررسید قفل شده به شکل باکس اطلاعاتی غیرقابل تغییر
...
if st.session_state.get("show_table", False):
    ...
    try:
        ...
        # ذخیره تاریخ‌های داینامیک در دیکشنری جهت اعمال به PDF خروجی
...
if st.session_state.get("show_table", False) and "final_res" in st.session_state:
    ...
    # نمایش جدول نهایی به کاربر با ستون‌های استاندارد و مرتب شده
    if table_data:
        import pandas as pd
        ...
        # تزریق CSS پیشرفته جهت رفع قطعی هاله طوسی لبه‌ها و هم‌ارتفاع کردن ردیف‌ها
        st.markdown("""
            <style>
                /* ۱. هدف قرار دادن تمام المان‌های دربرگیرنده جدول برای حذف کامل سایه و هاله طوسی */
                ...
                /* رفع مشکل هاله طوسی رنگ در بلاک والد اصلی استریم‌لیت */
                ...
                /* ۲. استایل عمومی سلول‌ها و قفل کردن دقیق ارتفاع ردیف‌ها برای هم‌اندازه شدن */
                div[data-testid="stTable"] th, div[data-testid="stTable"] td {
                    border: 1px solid #000000 !important;
                    text-align: center !important;
                    vertical-align: middle !important;
                    padding: 0px 6px !important; /* به صفر رساندن پدینگ عمودی برای کنترل دقیق ارتفاع */
                    height: 38px !important;     /* قفل کردن ارتفاع روی یک عدد ثابت */
                    max-height: 38px !important;
                    line-height: 38px !important;  /* تراز عمودی متن با ارتفاع ردیف */
                    box-sizing: border-box !important;
                }
                
                /* ۳. راست‌چین کردن اختصاصی بدنه ستون شرح کالا */
                div[data-testid="stTable"] td:first-child {
                    text-align: right !important;
                    padding-right: 15px !important;
                }
                
                /* ۴. وسط‌چین ماندن هدرها (شامل شرح کالا) */
        ...
        # رندر جدول و باکس موفقیت پرداخت
        st.table(df_display)
                ...
                try:
                    with st.spinner("در حال تولید نقشه چیدمان..."):
                        ...
                        for idx, room in enumerate(st.session_state.manual_rooms):
                            ...
                            # ساخت پلی‌لاین قوی‌تر برای تشخیص بهتر
                            ...
                            # اضافه کردن متن برای کمک به تشخیص

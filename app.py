...
def main(page: ft.Page):
    ...
    def purchase_request_page():
        product_data = {"گرمایش از کف": ["طول 1 متر", "طول 2 متر", "طول 3 متر"], "رادیاتور": ["50×50", "50×90", "60×60", "90×150", "90×200"], "حوله خشک کن": ["60×40", "80×50", "100×60"], "گرمکن مخزن": ["100 لیتری", "200 لیتری", "500 لیتری"], "عایق بازتابشی": ["3 مترمربع", "6 مترمربع", "10 مترمربع"]}
        product_name = ft.Dropdown(label="نام محصول", width=350, options=[ft.dropdown.Option(k) for k in product_data.keys()])
        ...
        return ft.Container(content=ft.Column([ft.Container(content=ft.Row([ft.IconButton(icon=ft.Icons.ARROW_BACK, on_click=lambda e: render(4)), ft.Text("ثبت درخواست خرید", size=20, weight="bold")]), padding=10), product_name, ft.ElevatedButton("بارگذاری ابعاد", on_click=load_sizes, bgcolor="#1565C0", color="white", width=350), product_size, product_qty, ft.ElevatedButton("افزودن به لیست", on_click=add_to_table, bgcolor="green", color="white", width=350), ft.Divider(), table, ft.ElevatedButton("ثبت نهایی درخواست خرید و دانلود PDF", on_click=generate_purchase_pdf, bgcolor="#1565C0", color="white", width=350, icon=ft.Icons.DOWNLOAD)], scroll=ft.ScrollMode.AUTO, spacing=15), width=400, expand=True, padding=15)
    ...
    def commission_page():
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("درصد همکاری ماهانه", size=18, weight="bold"),
                ft.Text("فاکتورهای تسویه شده: ۴۸,۵۰۰,۰۰۰ تومان", size=16),
    ...
    def update_page():
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("نسخه فعلی: ۱.۴.۳", size=18, weight="bold"),
                ft.Text("آخرین نسخه: ۱.۴.۵", size=16, color="blue"),
                ft.ElevatedButton("دانلود و نصب بروزرسانی", bgcolor="#1565C0", color="white", width=350, on_click=lambda e: show_message("در حال دانلود بروزرسانی...", "blue"))
            ], scroll=ft.ScrollMode.AUTO, spacing=20, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            width=400, expand=True, padding=15
        )
    ...
    def network_page():
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("نقشه شبکه فروش و خدمات\n(در نسخه کامل نقشه تعاملی قرار خواهد گرفت)", size=16, text_align=ft.TextAlign.CENTER)
            ], scroll=ft.ScrollMode.AUTO, spacing=20, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            width=400, expand=True, padding=15
        )
    ...
    def rules_page():
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text("متن قوانین و شرایط همکاری\nدر نسخه کامل اینجا قرار می‌گیرد", size=16, text_align=ft.TextAlign.CENTER)
            ], scroll=ft.ScrollMode.AUTO, spacing=20, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            width=400, expand=True, padding=15
        )
    ...
    # ==================== صفحات اصلی (بدون تغییر) ====================
    def dashboard_page():
        # کد داشبورد شما (کاملاً بدون تغییر)
